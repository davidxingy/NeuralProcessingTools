% script to add simulated spike counts from EMG data to serve as controls.
...
if downsampleFactor == 10
    load(fullfile(baseDir,'ProcessedData','NeuralFiringRates1msBins10msGauss.mat'),'allFRs');
end

emg1ms = downsampEMG;
cd(fullfile(baseDir,'ProcessedData'))

% get the corresponding EMG for each neuron time bin that we want
emgInds = round(NeurEMGSync((1:size(allFRs,2))*30,frameEMGSamples,frameNeuropixelSamples,'Neuropixel')/20);
emgInds(emgInds <= 0) = 1;
emgSynced = emg1ms(:,emgInds);
emgNormalized = emgSynced./prctile(emgSynced,99,2);

% now build simulated neural activity
nNeurons = size(allFRs,1);
nEMG = size(emgNormalized,1);
nTimePoints = size(emgNormalized,2);

% sample weights
emgWeights = randn(nNeurons,nEMG);
...
%calc poisson rate
timeVarRates = emgWeights*emgNormalized+baselines+segNoise;
