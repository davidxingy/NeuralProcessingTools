...
for iRegion = 1:size(pcaDiverCtx,2)
    ...
    for iAnimal = 1:size(umapDists,1)
        ...
        [~, freqRegionRank] = sort(squeeze(timeEmgDists(iAnimal,iRegion,:)));
        ...
        bestUmapCtxRank(iAnimal,iRegion) = find(ctxRegionRank == umapRegionRank(1));
        bestUmapStrRank(iAnimal,iRegion) = find(strRegionRank == umapRegionRank(1));

        bestFreqCtxRank(iAnimal,iRegion) = find(ctxRegionRank == freqRegionRank(1));
        bestFreqStrRank(iAnimal,iRegion) = find(strRegionRank == freqRegionRank(1));

        %get controls
        for iShuff = 1:nControlShuffs
            permInds = randperm(size(pcaDiverCtx,2)-1);
            ctxRankShuff = ctxRegionRank(permInds);
            strRankShuff = strRegionRank(permInds);

            bestUmapCtxRankShuff(iAnimal,iRegion,iShuff) = find(ctxRankShuff == umapRegionRank(1));
            bestUmapStrRankShuff(iAnimal,iRegion,iShuff) = find(strRankShuff == umapRegionRank(1));

            bestFreqCtxRankShuff(iAnimal,iRegion,iShuff) = find(ctxRankShuff == freqRegionRank(1));
            bestFreqStrRankShuff(iAnimal,iRegion,iShuff) = find(strRankShuff == freqRegionRank(1));
...
nexttile(2)
scatter(allUmaps,allCtxs,15,[0.3 0.3 0.3],'filled')
linReg = fitlm(allUmaps,allCtxs);
hold on
plot(get(gca,'XLim'),get(gca,'XLim')*linReg.Coefficients.Estimate(2)+linReg.Coefficients.Estimate(1),'Color',lines(1),'LineWidth',2,'LineStyle','--')
text(0.1,0.49,['R^2 = ' num2str(linReg.Rsquared.Ordinary)],'fontSize',14)
text(0.1,0.45,['CC = ' num2str(corr(allUmaps',allCtxs','type',corrType))],'fontSize',14)
...
nexttile(6)
scatter(allUmaps,allStrs,15,[0.3 0.3 0.3],'filled')
linReg = fitlm(allUmaps,allStrs);
hold on
plot(get(gca,'XLim'),get(gca,'XLim')*linReg.Coefficients.Estimate(2)+linReg.Coefficients.Estimate(1),'Color',lines(1),'LineWidth',2,'LineStyle','--')
text(0.1,0.8,['R^2 = ' num2str(linReg.Rsquared.Ordinary)],'fontSize',14)
text(0.1,0.7,['CC = ' num2str(corr(allUmaps',allStrs','type',corrType))],'fontSize',14)
xlabel('UMAP Distance')
ylabel('Striatum Divergence')
set(gca,'TickDir','out')
set(gca,'FontSize',13)
